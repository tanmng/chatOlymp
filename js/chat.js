...
var ajaxChat = {
	...
	onNewMessage: function(dateObject, userID, userName, userRole, messageID, messageText, channelID, ip) {
		...
		if(this.parseScoreMessageCommand(messageText)) {
			return false;
		}
	...
	/* Customize for arena.olympiavn.org 
	 * Update the scoreboard and return the announcement to chat log
	 */
	parseScoreMessageCommand: function(messageText) {
		if(messageText.indexOf('/score') === 0) {
			var score_string = messageText.substr(7);
			var score_json = JSON.parse(this.decodeSpecialChars(score_string));
			// console.log("string: " + score_string);
			// console.log(score_json);
		 	if (true)
		 	{
		 		// Display the name and score
		 		this.dom['score1'].innerHTML = score_json.team1.score;
		 		this.dom['score2'].innerHTML = score_json.team2.score;
		 		
				this.dom['name1'].innerHTML  = score_json.team1.name;
				this.dom['name2'].innerHTML  = score_json.team2.name;
		 	}
			
			// Return false so that we still display this message and it will be replace with appropriate text later
			return false;
		}
		return false;
	},
	...
	handleInputFieldKeyUp: function(event) {
		switch(event.which)
		{
			case 38:
				// Up arrow
				if (this.old_msg_index > 0)
				{
					// Set the previous message (by this index) to the field
					this.dom['inputField'].value = this.old_msg_list[--this.old_msg_index];
				}
				break;
			case 40:
				// Down arrow
				if (this.old_msg_index < this.old_msg_list.length)
				{
					if (this.old_msg_index < this.old_msg_list.length - 1)
						this.dom['inputField'].value = this.old_msg_list[++this.old_msg_index];
					else
					{
	...
	sendMessage: function(text) {
		...
		if(text) {
			// Add this message to list of old ones
			this.old_msg_list.push(text)
			this.old_msg_index = this.old_msg_list.length;
	...
	replaceCommands: function(text) {
		try {
			...
			switch(textParts[0]) {
				...
				/* Customize for arena.olympiavn.org */
				case '/score':
					return this.replaceCommandScore(text);
				case '/question':
					return this.replaceCommandQuestion(text);
				default:
	...
	/* Customize for arena.olympiavn.org */
	replaceCommandScore: function(text) {
		var score_string = text.substr(7);
		var score_json = JSON.parse(this.decodeSpecialChars(score_string));	
		console.log("Json from replace: ");
		// console.log("string: " + score_string);
		console.log(score_json);
		change_flag  = score_json.hasOwnProperty('changes');
		return '<span class="chatBotErrorMessage">' + this.lang['currentScore']
				+ '<b>' + score_json.team1.name + ' ' 
				+ score_json.team1.score + '</b>' 
				+ (change_flag? ' (' + (score_json.changes[0] >= 0? '+': '') + score_json.changes[0] + ')' : '')
				+ ' - '
				+ '<b>' + score_json.team2.name + ' '
				+ score_json.team2.score + '</b>' 
				+ (change_flag? ' (' + (score_json.changes[1] >= 0? '+': '') + score_json.changes[1] + ')' : '')
				+ '</span>';
	},
	
	/* Customize for arena.olympiavn.org
	 * Make a span text part that cannot be selected or copied
	 */
	replaceCommandQuestion: function(messageText)
	{
		return '<span class="chatBotQuestionMessage" >' + 'Question: ' 
	...
	replaceBBCodeImage: function(url) {
		...
		if(this.settings['bbCodeImages']) {
			...
			return	'<a oncontextmenu="return false;" onclick="return false;">'
					+'<img oncontextmenu="return false;" class="bbCodeImage" style="max-width:'
					+maxWidth
					+'px; max-height:'
					+maxHeight
					+'px;" src="'
