...
class AJAXChat {
		...
		$sql = 'INSERT INTO '.$this->getDataBaseTable('online').'(
          ...
          teamName,
				...
				VALUES (
					...
					'.$this->db->makeSafe($this->getTeamName()).',
	...
	function insertParsedMessage($text) {
        ...
        case '/saveprivate':
          $this->insertParsedMessageSavePrivate($textParts);
          break;
        ...
        // Isolate a team from outside private messages
        case '/isolate':
          $this->insertParsedMessageIsolate($textParts);
          break;
	...
	function insertParsedMessagePrivMsg($textParts) {
		if($this->isAllowedToSendPrivateMessage()) {
          ...
          /* Customize for arena.olympiavn.org
           * Implement the isolated list */
          $isolated_list = include(ISOLATED_LIST);
          $my_team = $this->getTeamName();
          $target_team = $this->getTeamNameFromUserID($toUserID);
          $priv_allow = true;   // flag if we can send private 
          if (in_array($target_team, $isolated_list))
          {
            // Target of this private message is in the isolated list, have to 
            // check carefully now
            if ($this->getUserRole == AJAX_CHAT_ADMIN || $this->getUserRole == AJAX_CHAT_MODERATOR || 0 == strcmp($my_team, $target_team))
              // In these case, user can send private
              $priv_allow = true;
            else
              $priv_allow = false;
          }
          ...
          {
            // Cannot send private message to this user now
            $this->insertChatBotMessage(
              $this->getPrivateMessageID(),
              'You cannot send private message to user '.$textParts[1] .' now!'
            );
          }
				}
			}
    ...
    /* 
     * Customize for arena.olympiavn.org
     * Return the name of the team for this user
     */
    function insertParsedMessageInWhatTeam($textParts) {
		if(count($textParts) == 1) {
			$this->insertChatBotMessage(
				$this->getPrivateMessageID(),
				'/error MissingUserName'
			...
			if($whereisUserID === null) {
                ...
                $teamName = $this->getTeamNameFromUserID($whereisUserID);
                $this->insertChatBotMessage(
                    $this->getPrivateMessageID(),
                    'User '.$textParts[1].' is in team --'.$teamName.'--'
                );	
			}
		}
	}
	...
	function insertParsedMessageNick($textParts, $announce_flag = true) {
		if(!$this->getConfig('allowNickChange') ||
			...
			/* Customize for arena.olympiavn.org */
				...
				$prefix = $this->getConfig('modNamePrefix');
				$suffix = $this->getConfig('modNameSuffix');
			} else if($this->getUserRole() == AJAX_CHAT_ADMIN) {
				$prefix = $this->getConfig('adminNamePrefix');
				$suffix = $this->getConfig('adminNameSuffix');
			}
			...
			if(!$newUserName) {
				...
				if($this->isUserNameInUse($newUserName)) {
					...
					/* Customize for arena.olympiavn.org */
					// Only announce it it's not a GUEST and the announce_flag is set
					if ($this->getUserRole() != AJAX_CHAT_GUEST && $announce_flag) {
						$this->insertChatBotMessage(
							$this->getChannel(),
	...
	/* Customize for arena.olympiavn.org 
	 * Enable / disable spectator to chat in public
	 */
	function insertParsedMessageSpectator($textParts)
	{
		if ($this->getUserRole() != AJAX_CHAT_ADMIN)
		{
		...
		else
		{
			if (count($textParts) != 2)
			{
				// Invalid, the command must either be '/spectator on' or '/spectator off'
				$this->insertChatBotMessage(
					$this->getPrivateMessageID(),
			...
			{
				// OK, valid command now, process the flag
				$flag = false;
				switch($textParts[1])
				{
					case 'on':
						$flag = true;
						$announcement = $this->getLang('spOn');
					break;
					case 'off':
						$flag = false;
						$announcement = $this->getLang('spOff');
					break;
					default:
						$this->insertChatBotMessage(
							$this->getPrivateMessageID(),
				...
				$spec_config_file = SPECTATOR_CONFIG_FILE_PREFIX . $current_channel .".php";
				// Save the config
				file_put_contents($spec_config_file, '<?php return '. var_export($flag, true) .';// Allow or forbid spectator (AJAX_CHAT_GUEST) from chating ?>');
				
				// Announce to the public about this
				$this->insertChatBotMessage(
					$this->getChannel(),
					$announcement
				);
	...
	/* Customize for arena.olympiavn.org
	 * Enable / disable a player to change his/her name
	 */
	function insertParsedMessageName($textParts)
	{
		if ($this->getUserRole() != AJAX_CHAT_ADMIN)
		{
			// This command is for admin only
		...
		else
		{
			...
			else
			{
				...
				switch($textParts[1])
				{
					case 'on':
						$flag = true;
						$announcement = $this->getLang('ncOn');
					break;
					case 'off':
						$flag = false;
						$announcement = $this->getLang('ncOff');
					break;
					default:
						$this->insertChatBotMessage(
							$this->getPrivateMessageID(),
				...
				// Save the config
				file_put_contents(NAME_CHANGE_FILE, '<?php return '. var_export($flag, true) .';// Allow or forbid player (AJAX_CHAT_USER) from change his/her display name ?>');
	...
	/* Customize for arena.olympiavn.org
	 * Announce a question via chatbot in a format that cannot be copied
	 */
	function insertParsedMessageQuestion($textParts)
	{
		if ($this->getUserRole() == AJAX_CHAT_GUEST)
		{
			// This command is for admin only
		...
		else
		{
			// Make the question:
			/*
			// Let's use a very basic encryption here - base64
			
			$question = implode(' ', array_slice($textParts, 1)); 
			$question_enc = base64_encode($question);	// ciphertext of the question
			*/
			$question_text = implode(' ', $textParts);; // The initial /question is necessary for the code in the client to process
            $question_text = $question_text.($this->getUserRole() == AJAX_CHAT_ADMIN? '' : ' [FROM --'.$this->getDisplayName().'--]');
	...
	/* Customize for arena.olympiavn.org
	 * Save all messages in this channel to a log file
	 */
	function insertParsedMessageSaveGame($textParts)
	{		
		if (($this->getUserRole() == AJAX_CHAT_ADMIN) || ($this->getUserRole() == AJAX_CHAT_MODERATOR && $this->getChannel() != 0 && $this->getChannel() != 1))
		{
			$current_channel = $this->getChannel();
			$all_messages = 'SELECT
					...
					WHERE
						channel= '. $current_channel .' ORDER BY dateTime ASC;';
		
			// Create a new SQL query:
			$result = $this->db->sqlQuery($all_messages);
			...
			// Open the log file to save 
			$log_file_name = GAME_LOG_DIR.'game_'.$current_channel.'_'.date('Y-m-d_G:i').'.txt';
			$log_file = fopen($log_file_name, "w");
			
			// Process the query result and save to file
			while($row = $result->fetch()) {
				$row_text = $row['text'];
				
				$line_text = $row_text;
				
				$line = '('. $row['dateTime'] .') '.$row['userName'] .': '. $line_text ."\r\n";
			...
			$log_file_name_parts = explode('/', $log_file_name);
			$log_file_link = 'http://arena.olympiavn.org/gamelogs/' . array_pop($log_file_name_parts);
			$result = 'The log of all messages untill now for this channel is saved to '. $log_file_link. '  Please use /deletelogs to delete all old logs of this channel to prevent other teams from downloading them';
			$this->insertChatBotMessage(
				$this->getPrivateMessageID(),
				$result
			);
		...
		{
			// This command is for admin in public channel and any mod in private channel
			$this->insertChatBotMessage(
				$this->getPrivateMessageID(),
				'/error CommandNotAllowed '.$textParts[0]
			);
   ...
   */
  function insertParsedMessageSavePrivate($textParts)
  {
		if ($this->getUserRole() == AJAX_CHAT_ADMIN)
		{
			$current_channel = $this->getChannel();
			$all_messages = 'SELECT
					...
					FROM
						'.$this->getDataBaseTable('messages').'
					WHERE
			      userID != '.$this->getConfig('chatBotID').' AND 
            text LIKE \'/privmsgto%\' AND 
						channel between 1000000000 and 9000000000 ORDER BY dateTime ASC;';
			...
			// Open the log file to save 
			$log_file_name = GAME_LOG_DIR.'private_'.date('Y-m-d_G:i').'.txt';
			$log_file = fopen($log_file_name, "w");
        ...
        $text_array = explode(' ', $row_text);
        $recipient = $text_array[1];

				$line = $row['dateTime'] .','.$row['userName'] . ','. $recipient .','. implode(' ', array_slice($text_array, 2)) ."\r\n";
				fwrite($log_file, $line);
			}
			...
			$log_file_name_parts = explode('/', $log_file_name);
			$log_file_link = 'http://arena.olympiavn.org/gamelogs/' . array_pop($log_file_name_parts);
			$result = 'The log of all private message up until now is saved in '. $log_file_link. '  Please use /deletelogs to prevent other teams from downloading them';
			$this->insertChatBotMessage(
				$this->getPrivateMessageID(),
				$result
			);
	...
	/* Customize for arena.olympiavn.org
	 * Delete all logs of current channel to prevent teams from spying on one another
	 */
	function insertParsedMessageDeleteLogs($textParts)
	{
		if (($this->getUserRole() == AJAX_CHAT_ADMIN) || ($this->getUserRole() == AJAX_CHAT_MODERATOR && $this->getChannel() != 0 && $this->getChannel() != 1))
		{
			// This command is for admins on all channels or mods on private channels
			$current_channel = $this->getChannel();
			$log_file_name_wildcard = GAME_LOG_DIR.'game_'.$current_channel.'_*.txt';
			$file_array = glob($log_file_name_wildcard);
			
			// Delete all those log files
			array_map( "unlink", $file_array);
			
			// Announce
			$result = 'All log files for this channel have been removed, however all messages are still in database, for security reason, only admins are allowed to remove messages from database';
			$this->insertChatBotMessage(
				$this->getPrivateMessageID(),
				$result
			);
  ...
  /* Customize for arena.olympiavn.org
   * Set a team to be isolated from outside private message
   */
  function insertParsedMessageIsolate($textParts)
  {
		if ($this->getUserRole() != AJAX_CHAT_ADMIN and $this->getUserRole() != AJAX_CHAT_MODERATOR)
		{
			// This command is for admin only
			$this->insertChatBotMessage(
				$this->getPrivateMessageID(),
      ...
      {
      case 'list':
        // Display current list of isolated teams
        $list = include(ISOLATED_LIST);
        $this->insertChatBotMessage(
          $this->getPrivateMessageID(),
          'List of isolated teams is: '.implode(', ', $list)
        );
        break;
      case 'add':
        if (count($textParts) < 3)
        {
          ...
          $this->insertChatBotMessage(
            $this->getPrivateMessageID(),
            '/error IncorrectSyntax '.$textParts[0]
          );
          return;
        }

        // Add one team to the list
        $list = include(ISOLATED_LIST);
        $team_name = $textParts[2];
        if (!in_array($team_name, $list))
        {
          $list[] = $team_name;
          // Save the list
          file_put_contents(ISOLATED_LIST, '<?php return '. var_export($list, true) .'; // List of isolated teams ?>');
          $this->insertChatBotMessage(
            $this->getPrivateMessageID(),
            'Team '.$team_name .' is set to be isolated!'
          );
        }
        else
        {
          ...
          $this->insertChatBotMessage(
            $this->getPrivateMessageID(),
            'This team is already in the list '.$team_name
          );
        }
        break;
      case 'delete':
        ...
        if (in_array($team_name, $list))
        {
          ...
          {
            if (strcmp($team_name, $list[$i]))
              // Not the team we want to delete from list
              $new_list[] = $list[$i];
          }
          // Save the list
          file_put_contents(ISOLATED_LIST, '<?php return '. var_export($new_list, true) .'; // List of isolated teams ?>');
          $this->insertChatBotMessage(
            $this->getPrivateMessageID(),
            'Team '.$team_name .' now free!'
          );
        }
        else
        {
          ...
          $this->insertChatBotMessage(
            $this->getPrivateMessageID(),
            'This team is already not in the list '.$team_name
          );
        }
        break;
      default:
	...
	/*
	 * Process the command to set or announce score of teams
	 */
	function insertParsedMessageScore($textParts)
	{
		if ($this->getUserRole() != AJAX_CHAT_ADMIN)
		{
			// This command is for admin only
		...
		else
		{
			...
			{
				case 'name':
					// Set the name of the 2 teams
					
					// Parse the name
					$name_string = implode(' ', array_slice($textParts, 2));
					...
					for ($i = 0; $i < count($names); $i++)
						$score_config["team".($i + 1)] = array("name" => trim($names[$i]), "score" => 0);
					
					//  Save the config to the file
					file_put_contents(SCORE_FILE, "<?php return ".  var_export($score_config, true) ." //Score of teams ?>");
					
					// Go right throught to the announce
				case 'announce':
					// Announce current score of 2 teams along with their score
					
					$score_data = include(SCORE_FILE);
					
					// Make the announcement
					$announcement = '/score '.json_encode($score_data);
				...
				case 'change':
					// Parse the change
					$score_change_string = implode(' ', array_slice($textParts, 2));
					$changes = explode(';', $score_change_string);
					
					if (count($changes) != 2 || !is_numeric($changes[0]) || !is_numeric($changes[1]))
					{
						$this->insertChatBotMessage(
							$this->getPrivateMessageID(),
							'/error IncorrectSyntax '.$textParts[0]
					...
					// Change the score of 2 teams
					$score_data = include(SCORE_FILE);
					
					// Update the file
					for ($i = 0; $i < count($changes); $i++)
						$score_data["team".($i + 1)]["score"] += floatval(trim($changes[$i]));
					
					// Save the new score
					file_put_contents(SCORE_FILE, "<?php return ".  var_export($score_data, true) ." //Score of teams ?>");
					
					// Make the announcement
					$new_score_data = $score_data;
					$new_score_data["changes"] = $changes;
					$announcement = '/score '.json_encode($new_score_data);
				...
				case 'set':
					// Set the score of the 2 teams
					// Parse the change
					$score_string = implode(' ', array_slice($textParts, 2));
					$scores = explode(';', $score_string);
					...
					for ($i = 0; $i < count($scores); $i++)
						$score_data["team".($i + 1)]["score"] = floatval(trim($scores[$i]));
	...
	/*
	 * Process command of clear old messages
	 */
	function insertParsedMessageClearAll($textParts)
	{
		if ($this->getUserRole() != AJAX_CHAT_ADMIN)
		{
			// This command is for admin only
		...
		{	
			...
			// Notice the admin back
			$this->insertChatBotMessage(
				$this->getPrivateMessageID(),
				$this->getLang('clearedAll')
			);
	...
	function removeInactive() {
		...
		if($result->numRows() > 0) {
			...
			while($row = $result->fetch()) {
				...
				if($this->getUserRole() != AJAX_CHAT_GUEST)
				{
					// We only announce when it's not about a guest (spectator)
	...
	function getDisplayName() {
		return $this->getSessionVar('DisplayName');
	}
	
	/* Customize for arena.olympiavn.org */
	function setDisplayName($displayName) {
		$this->setSessionVar('DisplayName', $displayName);
	}

    /* Customize for arena.olympiavn.org */
    function getTeamName()
    {
        return $this->getSessionVar('TeamName');
    }

    /* Customize for arena.olympiavn.org */
    // Process a userName and return the teamName
    function parseTeamName($user_name)
    {
        $suppose_team_name = strpos($user_name , '_');
        if ($suppose_team_name)
            return substr($user_name, 0, $suppose_team_name);
        else
            return 'UNDEFINED';
    }

    /* Customize for arena.olympiavn.org */
    function getTeamNameFromUserID($userID)
    {
		$userDataArray = $this->getOnlineUsersData(null,'userID',$userID);
		if($userDataArray && isset($userDataArray[0])) {
			return $userDataArray[0]['teamName'];
		}
		return null;
    }
    ...
    function setTeamName($teamName) {
        $this->setSessionVar('TeamName', $teamName);
    }
	...
	function getLang($key=null) {
		if(!$this->_lang) {
			...
			/* Customize for arena.olympiavn.org */
			require(AJAX_CHAT_PATH.'lib/lang/en.php');
			$this->_lang = &$lang;
		}
		if($key === null)
			return $this->_lang;
